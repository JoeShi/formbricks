...
基于简化后的设计文档，按增量方式实现 SSE 实时响应通知系统。仅在 `responseFinished` 时发布事件，移除心跳、指数退避和手动重连逻辑，依赖浏览器原生 EventSource 自动重连。

## 任务

- [ ] 1. 创建类型定义和事件总线核心模块
  - [ ] 1.1 创建 `modules/response-notification/lib/types.ts`，定义 `TResponseEvent`、`TResponseEventCallback`、`IResponseEventBus` 接口
    - 包含 `id`（UUIDv7）、`environmentId`、`surveyId`、`surveyName`、`event`（仅 `"responseFinished"`）、`response`、`timestamp` 字段
    - 定义 `TStreamEvent` 类型，`event` 字段仅为 `"responseFinished"`
    - 定义 `formatResponsePreview` 辅助函数
    - 不包含 `TConnectionStatus` 类型
    - 从 `@formbricks/types/responses` 导入 `TResponse` 类型
    - _需求: 1.1, 1.2, 1.4, 6.1, 6.2, 6.3, 6.4_

  - [ ] 1.2 创建 `modules/response-notification/lib/response-event-bus.ts`，实现 `EventEmitterBus`
...
- [ ] 3. 实现 SSE Route Handler
  - [ ] 3.1 创建 `apps/web/app/api/v1/client/[environmentId]/responses/stream/route.ts`
    ...
    - 无心跳机制
    - 监听 `request.signal` 的 `abort` 事件，清理订阅
    - 设置 `export const dynamic = "force-dynamic"`
    - _需求: 2.1, 2.2, 2.3, 2.4, 2.5, 3.1, 3.2, 3.3, 3.4, 3.5_

- [ ] 4. 实现客户端 Hook
  - [ ] 4.1 在 `modules/response-notification/lib/types.ts` 中确认 `TStreamEvent` 和 `formatResponsePreview` 已定义
  ...
  - [ ] 4.5 创建 `modules/response-notification/hooks/useResponseStream.ts`
    - 创建 EventSource 连接
    - 收到 `response` 事件时更新 `lastEvent` 状态
    - 组件卸载时关闭 EventSource
    - 依赖浏览器原生 EventSource 自动重连
    - 返回类型: `{ lastEvent: TStreamEvent | null }`
    - 无 connectionStatus、无 reconnect、无指数退避
...
- [ ] 6. 实现通知组件并集成到仪表盘
  - [ ] 6.1 创建 `modules/response-notification/components/ResponseNotificationProvider.tsx`
  ...
  - [ ] 6.2 在 `apps/web/app/(app)/environments/[environmentId]/layout.tsx` 中集成 `ResponseNotificationProvider`
...
- [ ] 7. Pipeline 集成
  - [ ] 7.1 修改 `apps/web/app/api/(internal)/pipeline/route.ts`，仅在 `responseFinished` 时发布事件
    - 导入 `responseEventBus` 和 `uuidv7`
    - 仅当 `event === "responseFinished"` 时调用 `responseEventBus.publish()`
    - `responseCreated` 事件不触发发布
...
- 已移除: 心跳机制、指数退避重连、TConnectionStatus、reconnect 函数、Property 4（重连退避上界）
